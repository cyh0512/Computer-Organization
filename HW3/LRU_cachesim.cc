...
void cache_sim_t::init()
{
  ...
  way_vector = new std::vector<int>[sets]();

  for(size_t i=0;i<sets;i++){ //初始化vector，預先填入ways個way index(from 0 ~ ways-1)。
    for(size_t j=0;j<ways;j++){
      way_vector[i].push_back(j);
    }
  }
...
uint64_t* cache_sim_t::check_tag(uint64_t addr)
{
  ...
  std::vector<int>::iterator hit_idx= way_vector[idx].begin();

  for (size_t i = 0; i < ways; i++){ //檢查該set的所有ways
    if (tag == (tags[idx*ways + i] & ~DIRTY)){ //在該set的第i個way時，發生cache hit

      for(size_t j=0;j<ways;j++){ //尋找vector中該set的所有ways中儲存i的index。
        if(way_vector[idx][j]==i)
          hit_idx=hit_idx+j;
      }
      //hit_idx=find(way_vector[idx].begin(),way_vector[idx].end(),i); 
      way_vector[idx].erase(hit_idx); //因為儲存i的block剛剛存取過了，所以將它移到最後面。
      way_vector[idx].push_back(i);
...
{
  size_t idx = (addr >> idx_shift) & (sets-1);
  size_t way = way_vector[idx][0]; //要替換掉的block index儲存在way_vector的最左邊元素內。

  uint64_t victim = tags[idx*ways + way];

  std::vector<int>::iterator it = way_vector[idx].begin();
  way_vector[idx].erase(it); //將LRU block(第一個vector)儲存的way index移到vector最後面。
  way_vector[idx].push_back(way);
