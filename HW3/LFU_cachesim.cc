...
void cache_sim_t::init()
{
  ...
  victim_map = new std::map<size_t, int>[sets]();
  victim_map_cnt = new int[sets]();
  read_accesses = 0;
  read_misses = 0;
  bytes_read = 0;
  write_accesses = 0;
...
{
  tags = new uint64_t[sets*ways]; //tags就是cache的block array
...
void cache_sim_t::print_stats()
{
  ...
  std::cout << name << " ";
  std::cout << "LFU Bytes Read:            " << bytes_read << std::endl;
  std::cout << name << " ";
  std::cout << "LFU Bytes Written:         " << bytes_written << std::endl;
  std::cout << name << " ";
...
uint64_t* cache_sim_t::check_tag(uint64_t addr)
{
  ...
  for (size_t i = 0; i < ways; i++)
    ...
    {
      //第一次access發生cache miss之後會替換入cache(就是tags[])，下一次access時就會cache hit，使用頻率(victim_map[idx][tag])就會+1。
      //檢查目前SET的各個way是否有符合的tag，如果有相同tag就表示cache hit，在map的目前set(index)的tag欄位+1(表示出現頻率+1)。
      victim_map[idx][tag]++;
      return &tags[idx*ways + i];
    }
    /*else
    {
      victim_map[idx].insert(std::pair<size_t,int>(tag,0)); //如果cache miss，就將新欄位(tag,0)加入map。 //miss後會執行victimize，block會換入cache(tags[])，若下次cache hit時就會讓頻率+1。
    }*/
  return NULL;
}
...
uint64_t cache_sim_t::victimize(uint64_t addr)
{
  ...
  if(victim_map_cnt[idx]<ways){ //cache初始化後，目前使用的index(set)的格子數量還沒滿，即數量小於#ways。
      way = victim_map_cnt[idx]++; //victim_map_cnt在計算目前用了幾個way(同一個set的格子數量)，因為該set被使用的way數量還沒滿，所以把要替換的way index設為下一個空的way index。
  } else {
      size_t find_min = 10000000; //find_min是目前找到的目標address最小使用頻率，
      for(int i = 0; i<ways; i++){
          int cur_idx = idx*ways+i;
          if(victim_map[idx][tags[cur_idx]]<find_min){ //cur_idx = idx*ways+i //檢查目標address區塊的使用頻率是否小於find_min(目前找到的最小頻率)
              find_min = victim_map[idx][tags[cur_idx]]; //將find_min的值更新為找到的更小使用頻率
              way = i; //將way更新為找到的更小使用頻率的way number。
          }
      }
  }

  uint64_t victim = tags[idx*ways + way];
  tags[idx*ways + way] = (addr >> idx_shift) | VALID; //更新cache內的tag欄位
  return victim;
}
